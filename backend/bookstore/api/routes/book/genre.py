...
def read_genre(genre_id: UUID, session: SessionDep = SessionDep):
    """Get a genre by ID."""
...
def edit_genre(session: SessionDep, genre_id: UUID, genre_in: GenreUpdate):
    """
    Update a genre by ID.
    """
    genre = update_genre(session=session, genre_id=genre_id, genre_in=genre_in)
    if not genre:
        raise HTTPException(status_code=404, detail="Genre not found")
    return GenreRead.from_orm(genre)
...
def remove_genre(genre_id: UUID, session: SessionDep = SessionDep):
